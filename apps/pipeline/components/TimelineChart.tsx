...
export function TimelineChart({ data, isLoading }: TimelineChartProps) {
  ...
  const maxValueM = Math.max(...points.map((p) => p.estimatedValueM), 1);
    ...
    <div className="flex-1 overflow-y-auto bg-[var(--color-surface-base)]">
      <div className="max-w-[1100px] mx-auto px-6 py-8">
        ...
        <div className="flex justify-between items-start mb-7 flex-wrap gap-4">
          <div>
            <h2 className="text-[16px] font-medium text-[var(--color-text-primary)] mb-1">
              Tenders per maand
            </h2>
        ...
        <div className="flex gap-0 h-[240px] mb-8">
          ...
          <div className="flex flex-col justify-between pr-2.5 w-9 shrink-0">
            {[1, 0.75, 0.5, 0.25, 0].map((frac) => (
              <div
                key={frac}
                className="text-[10px] text-[var(--color-text-muted)] text-right leading-none"
              >
          ...
          <div className="flex-1 relative border-b border-[var(--color-border-subtle)] overflow-hidden">
            ...
            <div className="absolute inset-0 flex items-end gap-[3px] px-1">
              {points.map((point) => {
                ...
                const valueHeight = (point.estimatedValueM / maxValueM) * 100;
                return (
                  <div
                    key={point.month}
                  ...
                  >
                    ...
                    <div className="absolute bottom-[calc(100%+8px)] left-1/2 -translate-x-1/2 bg-[var(--color-surface-raised)] border border-[var(--color-border-subtle)] rounded-[var(--radius-md)] px-2.5 py-1.5 text-[11px] text-[var(--color-text-primary)] whitespace-nowrap z-30 opacity-0 pointer-events-none transition-opacity group-hover:opacity-100 flex flex-col gap-0.5">
                      ...
                      <span>€{point.estimatedValueM} M</span>
                    </div>
                    {/* Bar pair */}
        ...
        >
          {[
            ...
            {
              label: "Geschatte totaalwaarde",
              value: `€${data.totalValueM.toLocaleString("nl-NL")} M`,
              sub: "publiek aanbesteed",
            },
            {
              label: "Gemiddeld per maand",
              value: Math.round(
                data.totalTenders / Math.max(data.timeline.length, 1),
              ).toLocaleString("nl-NL"),
              sub: "tenders",
            },
            {
              label: "Meest actieve provincie",
              value: data.byProvince[0]?.province ?? "—",
              sub: `${data.byProvince[0]?.activeTenderCount ?? 0} tenders`,
            },
          ].map((card) => (
            <div
              key={card.label}
              className="bg-[var(--color-surface-raised)] border border-[var(--color-border-subtle)] rounded-[var(--radius-lg)] px-4 py-3.5 flex flex-col gap-1"
            >
              <span className="text-[10px] text-[var(--color-text-secondary)] tracking-[0.06em]">
                {card.label}
              </span>
        ...
        <div className="bg-[var(--color-surface-raised)] border border-[var(--color-border-subtle)] rounded-[var(--radius-lg)] px-4 py-4">
          <h3 className="text-[13px] font-medium text-[var(--color-text-primary)] mb-1">
            Signaalprovinciën
          </h3>
          <p className="text-[11px] text-[var(--color-text-secondary)] leading-relaxed mb-3.5">
            Provincies met hoge woningvraag maar lage tenderactiviteit —
            mogelijke capaciteitsknelpunten
          </p>
          <div className="flex flex-col gap-2">
            {data.byProvince
              ...
              .map((p) => (
                <div
                  key={p.province}
                  className="flex items-center gap-2.5 py-1.5 border-b border-[var(--color-border-subtle)] last:border-b-0"
                >
                  <div
                    className="w-1.5 h-1.5 rounded-full shrink-0"
                    style={{
                      background:
                        p.bottleneckSignal === "alert"
            ...
            {data.byProvince.filter((p) => p.bottleneckSignal !== "ok")
              .length === 0 && (
              <p className="text-[11px] text-[var(--color-text-secondary)]">
                Geen knelpunten gedetecteerd in huidige dataset.
              </p>
            )}
          </div>
        </div>
